    ...
    match mem::take(&mut inter.data) {
        Some(InteractionData::ApplicationCommand(command_data)) => {
            if inter.kind != InteractionType::ApplicationCommand {
                warn!("Received Interaction with type {} but data is ApplicationCommand", inter.kind.kind());
                return;
            }
        ...
        Some(InteractionData::MessageComponent(component_data)) => {
            if inter.kind != InteractionType::MessageComponent {
                warn!("Received Interaction with type {} but data is MessageComponent", inter.kind.kind());
                return;
            }
...
async fn handle_message_components(inter: Interaction, component_data: MessageComponentInteractionData, context: Ctx) {
    debug!("Received Message Component Interaction");

    match (component_data.component_type, component_data.custom_id.as_str()) {
